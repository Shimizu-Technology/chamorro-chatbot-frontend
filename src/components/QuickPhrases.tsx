        ...
        <div className="flex sm:hidden gap-2 overflow-x-auto -mx-3 px-3 scrollbar-hide snap-x snap-mandatory">
          {phrases.map((phrase, index) => (
            <button
              ...
              disabled={disabled}
              className="flex-shrink-0 w-36 px-2.5 py-2 text-[11px] bg-cream-100 dark:bg-gray-800 text-brown-700 dark:text-gray-300 border border-cream-300 dark:border-gray-700 rounded-xl hover:bg-teal-50 dark:hover:bg-ocean-950/30 hover:border-teal-300 dark:hover:border-ocean-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-left flex items-center gap-2 group snap-start"
            >
              <span className="text-sm group-hover:scale-110 transition-transform flex-shrink-0">{phrase.icon}</span>
              <span className="truncate leading-tight">{phrase.text}</span>
            </button>
          ))}
        </div>
        {/* Desktop: Grid */}
